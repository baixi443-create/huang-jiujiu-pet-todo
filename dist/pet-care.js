...
function draw(){q('#moodValue').textContent=Math.round(mood)+'%';q('#moodBar').style.width=mood+'%';const tags=q('#pet .petcard .meta + div');const feeling=mood>=75?'🥰 心情很好':mood>=50?'🙂 想和你待一会儿':'😭 心情很糟糕';const hungry=window.petFullness.get()<50?'🍽️ 好饿好饿！':'陪着你就很安心';tags.innerHTML='<span class="tag">'+feeling+'</span><span class="tag">'+hungry+'</span>';const label={calm:'悠闲陪伴',pending:'陪你努力中',overdue:'有点惦记你的任务'}[tier(snapshot(),Date.now())];q('#careStatus').textContent=label;if(window.renderPetVisual)window.renderPetVisual()}
...
const help=document.createElement('details');help.className='story';help.innerHTML='<summary>照顾啾啾 · <span id="careStatus"></span></summary><p>本地时间 18:00 至次日 09:00，每小时自然消耗：悠闲时饱食度 −1、心情 −0.6；有今日待办时 −1.2、−0.8；有逾期任务时 −1.5、−1。09:00 至 18:00 按上述速度的两倍消耗。跨时间段分段计算，三档不叠加，未来任务提前不会加速。</p><p>离线最多计算 24 小时；自然消耗最低保留饱食度 20、心情 25。喂食和玩具能帮啾啾恢复状态。</p>';q('#pet').append(help);

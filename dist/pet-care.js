...
function draw(){q('#moodValue').textContent=Math.round(mood)+'%';q('#moodBar').style.width=mood+'%';const tags=q('#pet .petcard .meta + div');const feeling=mood>=75?'🥰 心情很好':mood>=50?'🙂 想和你待一会儿':'🥺 有点想你了';const hungry=window.petFullness.get()<40?'有点饿，想吃点东西':'陪着你就很安心';tags.innerHTML='<span class="tag">'+feeling+'</span><span class="tag">'+hungry+'</span>';const label={calm:'悠闲陪伴',pending:'陪你努力中',overdue:'有点惦记你的任务'}[tier(snapshot(),Date.now())];q('#careStatus').textContent=label}
